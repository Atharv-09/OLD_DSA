...
ArrayList<Integer> ans = new ArrayList<>();
    ...
    static void find(Node root,int level,int[] arr){
    ...
    static int findHei(Node root){

        if(root==null) return 0;

        int lef = findHei(root.left);
        int rig = findHei(root.right);

        if(lef>rig) return lef+1;
        else return rig+1;

    }
    public static void solve(Node A) {
